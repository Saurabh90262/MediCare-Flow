...
/* -- 8. PHONE-NATIVE PANEL SURFACES -------------------------------------
   Part 7 gave phones their own navigation shell. Part 8 rebuilds what sits
   INSIDE it, surface by surface, because a bottom tab bar wrapped around
   desktop content is still desktop content. Every block below is a different
   ARRANGEMENT of the same controls, never the desktop one at reduced scale.

   It also repairs two cascade faults in part 7:
     - .stat b never applied, because .stat-txt b (part 6d) is more specific,
       so the compact statistic number silently kept its larger size;
     - .modal-foot .btn kept flex:1 1 44% after part 4g turned the footer into
       a column, where a percentage basis is read as a HEIGHT - the dialog
       buttons grew tall on the smallest phones.
   ---------------------------------------------------------------------- */
@media (max-width:600px){
  /* 8a. statistics: caption, number, hint - stacked, equal height, wrapping */
  .stat{flex-direction:column; align-items:flex-start; gap:9px; padding:14px 13px}
  .stat .stat-txt{width:100%; min-width:0}
  .stat .stat-txt small{font-size:10px; line-height:1.3; letter-spacing:.04em; white-space:normal}
  .stat .stat-txt b{font-size:clamp(22px,6.4vw,27px)}
  .stat .stat-txt span{display:block; font-size:11px; line-height:1.35}
  .stat .tap-cue{font-size:9.5px; margin-top:6px}

  /* 8b. overview progress: the value sits under its label, never beside it */
  .bar-row .bar-top{flex-direction:column; align-items:flex-start; gap:3px; font-size:12.5px; margin-bottom:8px}
  .bar-row .bar-top b{font-size:13.5px}
  .split-legend{flex-direction:column; gap:7px; margin-top:10px; font-size:12px}
  .legend{gap:9px 14px; font-size:11.5px}

  /* 8c. appointments as real record cards: caption above value, left aligned,
         token and identity on top, badges on tidy rows, actions last ------- */
  .tbl tbody tr{padding:12px 13px}
  .tbl td{flex-direction:column; align-items:flex-start; justify-content:flex-start; text-align:left; gap:3px; padding:7px 0}
  .tbl td::before{font-size:9.5px; letter-spacing:.09em; padding-top:0}
  .tbl td.tok{flex-direction:row; align-items:center; gap:11px; padding:0 0 11px}
  .who,.contact{width:100%; text-align:left}
  .tbl td .who b{font-size:14.5px; line-height:1.3}
  .tbl td .who span,.tbl td .contact span{font-size:12px}
  .tbl td[data-label='Date'],.tbl td[data-label='Quota'],
  .tbl td[data-label='Status'],.tbl td[data-label='Source']{flex-direction:row; align-items:center; justify-content:space-between; gap:10px; padding:5px 0}
  .tbl td[data-label='Actions']{padding:0}
  .tbl td[data-label='Actions']::before{display:none}

  /* 8d. analytics: a shorter chart, readable tiles, scroll stays inside ---- */
  .an-tile small{font-size:10.5px; line-height:1.3; white-space:normal}
  .an-tile p{font-size:11.5px; line-height:1.45}
  .trend{height:158px; gap:6px; padding:12px 2px 0}
  .trend-col{min-width:26px}
  .trend-lbl{font-size:9px}

  /* 8e. live queue status: hero figures two-up, legend two-up, equal-width
         toggles, and patient names get two lines instead of one clipped word */
  .lq-pills{display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:14px}
  .lq-pill{justify-content:center; padding:8px 10px; font-size:11.5px}
  .lq-pills > :last-child:nth-child(odd){grid-column:1/-1}
  .lq-dial small{font-size:9.5px; letter-spacing:.12em}
  .lq-bar{flex-direction:column; align-items:stretch; gap:12px}
  .lq-legend{display:grid; grid-template-columns:1fr 1fr; gap:8px 10px}
  .lq-bar .row{width:100%; display:grid; grid-template-columns:1fr 1fr; gap:8px}
  .lq-toggle{justify-content:center; min-height:44px; font-size:12px}
  .lq-cap{white-space:normal; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden; text-overflow:clip; line-height:1.25; font-size:10px}
  /* the dashed pointers must not bleed into the neighbouring pebble */
  .lq-peb.next{outline-width:2px; outline-offset:3px}
  .lq-peb.mine{outline-width:3px; outline-offset:3px}

  /* 8f. clinic settings: one column of titled cards, full-width controls -- */
  .set-grid > .panel{min-width:0}
  .panel-body.form-grid{gap:14px}
  .up-actions .btn{flex:1 1 auto; justify-content:center}
  .copy-row{flex-direction:column; align-items:stretch; gap:9px; font-size:12.5px}
  .copy-row .btn{width:100%; justify-content:center}
  .qrc-wrap{gap:14px}
  .qrc-side{width:100%; min-width:0}
  .qrc-side .btn{width:100%; justify-content:center}
}
@media (max-width:420px){
  /* 8g. a percentage flex basis in a column footer is a height, not a width */
  .modal-foot{flex-direction:column-reverse; gap:8px}
  .modal-foot .btn{flex:0 0 auto; width:100%; min-width:0}
}
@media (max-width:380px){
  /* 8h. the 320-375 band: two-up stops paying for itself on the wordy rows */
  .lq-pills,.lq-legend{grid-template-columns:1fr}
  .lq-bar .row{grid-template-columns:1fr}
  .tbl td .contact span{font-size:11.5px}
}
/* -- 5c. print ----------------------------------------------------------- */
@media print{
